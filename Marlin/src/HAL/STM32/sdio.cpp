...
#include "../platforms.h"
    ...
    */
		...
		#if (defined(SDIO_POLLING) || defined(STM32F103xE) || defined(STM32F103xG))
			// use polling mode
      ...
      */
      static bool SDIO_ReadWriteBlock(uint32_t block, const uint8_t *src, uint8_t *dst) {
        ...
        if (src) {
          ret = HAL_SD_WriteBlocks(&hsd, (uint8_t*)src, block, 1, SD_TIMEOUT);
        }
        else {
      ...
      */
      bool SDIO_ReadBlock(uint32_t block, uint8_t *dst) {
        ...
        while (retries--) if (SDIO_ReadWriteBlock(block, nullptr, dst)) return true;
        return false;
      ...
      */
      ...
      bool SDIO_WriteBlock(uint32_t block, const uint8_t *src) {
        ...
        uint8_t retries = SDIO_READ_RETRIES;
        while (retries--) {
          if (SDIO_ReadWriteBlock(block, src, nullptr)) return true;
          delay(10);
        }
        return false;
      ...
      */
      bool SDIO_ReadBlock(uint32_t block, uint8_t *dst) {
          ...
          uint8_t retries = SDIO_READ_RETRIES;        while (retries--) if (SDIO_ReadWriteBlock_DMA(block, nullptr, dst)) return true;
          return false;
